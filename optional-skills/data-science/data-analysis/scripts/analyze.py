"""
Analyze Excel (.xlsx/.xls) and CSV files using DuckDB.

Upstream: bytedance/deer-flow skills/public/data-analysis (MIT).
Hermes: cache under HERMES_HOME/.data-analysis-cache/.
"""
...
def _hermes_cache_dir() -> str:
    ...
    cache = base / ".data-analysis-cache"
    cache.mkdir(parents=True, exist_ok=True)
    return str(cache)


def _ensure_deps() -> None:
    global _duckdb, _openpyxl
    ...
    try:
        ...
        subprocess.run(
            [sys.executable, "-m", "pip", "install", "duckdb", "openpyxl", "-q"],
            check=True,
        )
        import duckdb as _duckdb_mod
...
def get_cache_db_path(files_hash: str) -> str:
    return os.path.join(_hermes_cache_dir(), f"{files_hash}.duckdb")


def get_table_map_path(files_hash: str) -> str:
    return os.path.join(_hermes_cache_dir(), f"{files_hash}{TABLE_MAP_SUFFIX}")
...
def load_files(con: Any, files: list[str]) -> dict[str, str]:
    """Load Excel/CSV files into DuckDB tables."""
    _ensure_deps()
    has_excel = any(os.path.splitext(f)[1].lower() in (".xlsx", ".xls") for f in files)
    if has_excel:
...
def _load_excel(con: Any, file_path: str, table_map: dict[str, str]) -> None:
...
def _load_csv(con: Any, file_path: str, table_map: dict[str, str]) -> None:
...
def action_inspect(con: Any, table_map: dict[str, str]) -> str:
