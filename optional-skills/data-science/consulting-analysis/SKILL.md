...
description: Consulting reports in two phases (framework + synthesis).
version: 1.0.0
...
metadata:
  hermes:
    tags: [data-science, research, consulting, reports]
    category: data-science
    related_skills: [data-analysis, chart-visualization, jupyter-live-kernel]
---

# Professional Research Report Skill

Ported from [DeerFlow `consulting-analysis`](https://github.com/bytedance/deer-flow/tree/main/skills/public/consulting-analysis).

Install: `hermes skills install official/data-science/consulting-analysis`

## Hermes skill mapping

| DeerFlow skill | Hermes equivalent |
|----------------|-------------------|
| `deep-research` | `web_search` + `web_extract` |
| `data-analysis` | `/data-analysis` (this repo optional skill) |
| `chart-visualization` | `/chart-visualization` (optional skill) |
| `web search agents` | `web_search` |
| `image-generation` | `image_gen` tool (if enabled) |

Load companion skills before Phase 2 when charts or table data are required.
...
After the analysis framework is generated, it is handed off to **other data collection skills** (e.g., `web_search`, `data-analysis`) to:
...
>
> **Chart Generation**: If a visualization skill is available (e.g., `chart-visualization`), chart generation can be deferred to the beginning of Phase 2 — see Step 2.3.
...
3. **Delegate to Visualization Skill**: Invoke `chart-visualization` for each chart task with:
...
- This skill operates in **two phases** of a multi-step agentic workflow:
  ...
  - **Data collection** is performed by other skills (`web_search`, `data-analysis`, etc.)
